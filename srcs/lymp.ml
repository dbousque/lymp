...
and deserialize py doc =
	...
	| "t" -> Pytuple (deserialize_list py (Bson.get_list element))
...
let get_tuple callable func args =
	match get callable func args with
	| Pytuple t -> t
	| ret -> raise (make_wrongtype callable func "tuple" ret)
...
let attr_tuple callable name =
	match attr callable name with
	| Pytuple t -> t
	| ret -> raise (make_wrongtype callable name "tuple" ret)
